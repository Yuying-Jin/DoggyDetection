...
export const FieldGuideConformedScreen = () => {
    ...
    const loadResultsFromStorage = async () => {
        ...
        if (jsonResults) {
            ...
            console.log("confirm screen: ", confirmResults)
        }
    };
    ...
    const deleteDog = async (dog) => {
        const updatedConfirmResults = confirmResults.filter((d) => d !== dog);
        setConfirmResults(updatedConfirmResults);
        const updatedDogs = [...updatedConfirmResults, ...notConfirmResults];
